...
public class ControladorPublico {
    private final ServicioCatalogoPublico catalogo; private final ServicioDisponibilidad disponibilidad; private final ServicioCitas citas; private final ServicioCancelacion cancelacion;
    public ControladorPublico(ServicioCatalogoPublico c,ServicioDisponibilidad d,ServicioCitas ci,ServicioCancelacion ca){catalogo=c;disponibilidad=d;citas=ci;cancelacion=ca;}
    ...
    public ResponseEntity<CitaCreadaRespuesta> crear(@PathVariable String slug,@Parameter(description="Clave única del intento de reserva",required=true) @RequestHeader("Idempotency-Key") String clave,@Valid @RequestBody SolicitudCrearCita solicitud){CitaCreadaRespuesta r=citas.crear(slug,clave,solicitud);return ResponseEntity.status(r.repetida()?HttpStatus.OK:HttpStatus.CREATED).body(r);}
    ...
    private String obtenerIp(HttpServletRequest p){String reenviada=p.getHeader("X-Forwarded-For");return reenviada==null?p.getRemoteAddr():reenviada.split(",")[0].trim();}
}
