...
public class ControladorAdministracionCitas {
    ...
    @PostMapping @Operation(summary="Crear una cita manualmente") public ResponseEntity<CitaCreadaRespuesta> crear(@AuthenticationPrincipal UsuarioAutenticado u,@RequestHeader("Idempotency-Key") String clave,@Valid @RequestBody SolicitudCrearCita s){if(u.getProfesionalId()!=null&&!u.getProfesionalId().equals(s.profesionalId()))throw new AccesoNoAutorizadoExcepcion("No puedes crear citas para otro profesional.");return ResponseEntity.status(HttpStatus.CREATED).body(citas.crear(u.getBarberiaSlug(),clave,s));}
