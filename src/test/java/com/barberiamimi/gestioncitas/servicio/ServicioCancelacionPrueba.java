...
class ServicioCancelacionPrueba {
 ...
 @BeforeEach void preparar()throws Exception{reset(catalogo,citas,notificaciones,intentos);propiedades=new PropiedadesAplicacion();propiedades.getCancelacion().setSecretoHmac("secreto-de-prueba-muy-largo-123456");barberia=new Barberia("Mimi","mimi");id(barberia,1L);Profesional p=new Profesional(barberia,"Mimi","mimi");id(p,2L);Servicio s=new Servicio(barberia,"Corte",null,new BigDecimal("15"),30);id(s,3L);cita=new Cita(barberia,p,s,"Ana","600123123",null,LocalDateTime.now().plusDays(3),LocalDateTime.now().plusDays(3).plusMinutes(30),null,null,"clave","huella");id(cita,10L);cita.setCodigoCancelacionHmac(crypto.hmac("01234:10",propiedades.getCancelacion().getSecretoHmac()));when(catalogo.buscarBarberia("mimi")).thenReturn(barberia);when(intentos.findTopByBarberiaIdAndHuellaOrigenAndExitosoFalseOrderByFechaIntentoDesc(anyLong(),anyString())).thenReturn(Optional.empty());servicio=new ServicioCancelacion(catalogo,citas,notificaciones,intentos,crypto,propiedades);}
 @Test @DisplayName("Debe cancelar una cita con teléfono y código correctos") void debeCancelar(){when(citas.findByBarberiaIdAndTelefonoClienteAndEstadoInAndAnonimizadaFalse(eq(1L),eq("600123123"),any())).thenReturn(List.of(cita));var r=servicio.cancelar("mimi",new SolicitudCancelarCita("600123123","01234"),"127.0.0.1");assertEquals("CITA_CANCELADA",r.codigo());assertEquals(EstadoCita.CANCELADA_POR_CLIENTE,cita.getEstado());assertNull(cita.getCodigoCancelacionHmac());verify(notificaciones).save(any(Notificacion.class));verify(intentos).save(any(IntentoCancelacion.class));}
